...
#| label: fig-bias-hse
#| out.width: 100%
#| fig-cap: "Observed differences between the unweighted 2021 HSE sample and 2021 Census."
#| echo: false
#| eval: true
...
Also, Andrew Heiss's detailed posts on working with logistic regression multilevel model outputs -- [here](https://www.andrewheiss.com/blog/2021/11/10/ame-bayes-re-guide/#posterior-predictions) and [here](https://www.andrewheiss.com/blog/2023/08/12/conjoint-multilevel-multinomial-guide/#bayesian-comparisonscontrasts) -- are an excellent resource, especially as a key ambition of the *SMART-MRP* is to predict out over different combinations of demographic and geographic subsets, and communicate uncertainty ranges to decision-makers.
...
1. Generate posterior predictions for each level in the model and look at how narrow the [Credible Intervals](https://en.wikipedia.org/wiki/Credible_interval) are for those values. This is using [marginal effects](https://marginaleffects.com/bonus/brms.html). 


2. Generate posterior conditional effects for each level, again using [marginal effects](https://marginaleffects.com/bonus/brms.html). 
...
#| echo: false
#| eval: false
# If we expect 75% to be reporting good outcomes, then the incidence of very bad outcomes is likely to be very small, less than 5%. We anchor intercept prior for bad health on a 2.5% probability of outcome $p/(1-p)$, so $log(0.025/0.975) ~ -3.66$, which in 
p <- tibble(n = rnorm(1e6, mean = -3.66, sd = 1)) |>  
  mutate(p = brms::inv_logit_scaled(n)) |> 
  ggplot(aes(x = p)) +
  geom_histogram(fill="#636363", binwidth = .02, boundary = 0, alpha=.5) +
  scale_y_continuous(breaks = NULL) +
...
#| label: fig-prior
#| out.width: 70%
#| fig-cap: "A prior expectation for the likelihood HSE respondents stating a *good* health outcome."
#| echo: false
#| eval: true
...
#| echo: false
#| eval: false
hse_stats <- hes_for_model |>
  group_by(health) |> 
  summarise(count=n()) |> ungroup() |> 
  mutate(
    prop=count/sum(count),
    ci=1.96*sqrt((prop * (1-prop)) / 5000)) |>  
  left_join(health |> summarise(across(-zone_id, ~sum(.))) |>
               pivot_longer(everything(), names_to="health",  values_to="count") |>
               mutate(census=count/sum(count)) |> 
               select(health, census)) |> 
  mutate(health=factor(health, levels=c("verygood", "good", "fair","bad", "verybad")))

p <- hse_stats |>
  mutate(health=fct_rev(health)) |> 
  ggplot(aes(y=health, x=prop)) +
  stat_gradientinterval(
    aes(dist = dist_normal(mu=prop, sigma=ci/2)),
    scale = .5, fill_type = "gradient",
    show_point = FALSE, show_interval = FALSE, fill="#636363" 
    ) +
    geom_spoke(aes(x=census), angle=get_radians(90), radius=.3, position="center_spoke",alpha=1, linewidth=.5, colour="#08519c") +
  labs(x="% in <span style='color: #08519c;'>**Census**</span> vs <span style='color: #636363;'>**HSE**</span> estimate", y="") + 
  scale_x_continuous(labels = scales::label_percent(), limits = c(0,.5), breaks=c(.1,.3,.5)) +
  theme(
    axis.title=element_markdown(size=40), 
    axis.text.x=element_text(size=30),  axis.text.y=element_text(size=30))
ggsave(here("figs", "hse_abs_error.png"), p, width=4.5, height=2.2, dpi=300)
```
...
#| eval: false
set.seed(13)
# Time difference of 1.53914 mins
start <- Sys.time()
model <-
  brm(data = hes_for_model,
      family = bernoulli(),
      is_vbad ~ sex + (1 | age2) + (1 | educ) + (1 | imd) + (1 | region) + (1 | is_urban), #+ (1 | educ:age2),
      #is_good ~ sex + (1 | age),
      prior = c(set_prior("normal(-3.66, 1)", class = "Intercept"),
...
# saveRDS(model, file = here("models", "fit_vbad.rds"))
model_vbad <- readRDS(here("models", "fit_vbad.rds"))
```
...
#| eval: false
# Very bad outcome.
plot_data |> filter(model=="modelVB") |> 
  filter(!(educ == "na/other" & condition == "educ")) |> 
  ggplot(aes(x = draw, y = cat_val)) +
  stat_gradientinterval(
    slab_size = 3,  
  ...
  scale_x_continuous(labels = scales::label_percent(), breaks=c(0,.04,.08)) +
  labs(
    x = "Predicted probability of **very bad** health", y = NULL,
    title = "Predicting over demographics by **model**") +
  theme(panel.grid.minor = element_blank(), panel.grid.major.y = element_blank()) +
  geom_vline(xintercept=.777, colour="#525252", linewidth=.2) +
...
#| out.width: 100%
#| fig-cap: "Predicted probabilities, given our model, of a good health outcome for the typical respondent in each demographic and areal-level grouping."
#| echo: false
#| eval: true
...
p <- plot_data |> filter(model=="modelVB") |> 
 ggplot(aes(x = draw, y = cat_val)) +
  ...
  scale_x_continuous(labels=scales::label_percent(), limits=c(-.04,.04)) +
  facet_grid(variable~model, scales="free_y", space="free_y") +
  ...
  #facet_grid(variable~., scales="free_y", space="free_y") +
   labs(
    ...
    title = "Posterior 'marginal effects'",
    #subtitle = "-- moving between levels of explanatory variables",
    x="% change in **very bad** health from control level", y=""
  ) +
  theme(
    axis.title = element_markdown(size=35),
    plot.title = element_markdown(size=60),
...
#| eval: false
# Fetch FMF results
fmf_results <- read_csv(here("data", "GH_ALL.csv")) |> 
...
#| label: fig-models
#| out.width: 100%
#| fig-cap: "Summary of model performance comapring MRP, with spatial microsimulation (default SAE technique in GIScience) for simulating good health at MSOA level."
#| echo: false
#| eval: true
